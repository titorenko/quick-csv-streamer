...
/** Double parser adopted from sun.misc.FloatingDecimal that can parse byte arrays directly without converting
 * them to strings 
 */
class JDKDoubleParser  {
    ...
    JDKDoubleParser(boolean negSign, int decExponent, byte[] digits,
            int n, boolean e) {
        isNegative = negSign;
        isExceptional = e;
     ...
     */
    private JDKDoubleParser(double d) {
    ...
    private void dtoa(int binExp, long fractBits, int nSignificantBits) {
         ...
         * and call it decExp. -- use more precise constants here. It
         * costs no more.
         */
         ...
         * takes account of the asymmetry of the spacing of floating-point
         * numbers below perfect powers of 2 26 Sept 96 is not that day. So we
         * use a symmetric test.
         */
        byte digits[] = this.digits = new byte[18];
        int ndigit = 0;
        boolean low, high;
         ...
         * asymmetry of the spacing of floating-point numbers below perfect
        ...
        if (Bbits < 64 && tenSbits < 64) {
            if (Bbits < 32 && tenSbits < 32) {
                ...
                if ((q == 0) && !high) {
                    ...
                    digits[ndigit++] = (byte) (0x30 + q);
                }
                /*
                 * HACK! Java spec sez that we always have at least one digit
                 * after the . in either F- or E-form output. Thus we will need
                 * more than one digit if we're using E-form
                 */
    ...
    static JDKDoubleParser readJavaFormatString(byte[] in, int startIndex, int length)
        ...
        parseNumber: try {
            if (length == 0)
                throw new NumberFormatException("empty array");
            int i = startIndex;
            ...
            if (c == 'N' || c == 'I') { // possible NaN or infinity
                ...
                byte targetChars[] = null; // char array of "NaN" or "Infinity"
                ...
                // and i must equal l
                if ((j == targetChars.length) && (i == end)) { // return NaN or
                                                                // infinity
                    return (potentialNaN ? new JDKDoubleParser(Double.NaN) // NaN
                                                                        // has
                                                                        // no
                                                                        // sign
                            : new JDKDoubleParser(
            ...
            if (i < end
                    && ((i != end - 1) || (in[i] != 'f'
                            && in[i] != 'F' && in[i]!= 'd' && in[i] != 'D'))) {
                break parseNumber; // go throw exception
            }

            return new JDKDoubleParser(isNegative, decExp, digits, nDigits, false);
        } catch (StringIndexOutOfBoundsException e) {
        }
        throw new NumberFormatException("For string: \"" + new String(in, startIndex, length) + "\"");
    }
    ...
    strictfp double doubleValue() {
        ...
        if (digits == infinity || digits == notANumber) {
            ...
            /*
             * dValue is now approximately the result. The hard part is
             * adjusting it, by comparison with FDBigInt arithmetic. Formulate
             * the EXACT big-number result as bigD0 * 10^exp
             */
            ...
            correctionLoop: while (true) {
                 ...
                 * Naively, we multiply by powers of ten and powers of two. What
                 * we actually do is keep track of the powers of 5 and powers of
                 * 2 we would use, then factor out common divisors before doing
                 * the work.
                 */
     ...
     * double/float.
     */
    protected static final double small10pow[] = { 1.0e0, 1.0e1, 1.0e2, 1.0e3,
...
class FDBigInt {
    ...
    public FDBigInt(long seed, byte digit[], int nd0, int nd) {
    ...
    public int quoRemIteration(FDBigInt S) throws IllegalArgumentException {
        ...
        if (diff != 0L) {
            ...
            while (sum == 0L) {
                 ...
                 * carry-out, or +1.
                 */
                assert sum == 0 || sum == 1 : sum; // carry out of division
                                                    // correction
                q -= 1;
            }
        }
